...
export default {
  ...
  data () {
    return {
      texto: 'Página de Restaurantes LIST',
      restaurantes: null,
      showBorrar: null
    }
  },
  methods: {
  	getRestaurantes(){
  		axios.get('http://localhost/restaurantes-vue/api-rest/restaurantes-api.php/restaurantes')
  			 .then((respuesta) => {
  	...
  	confirmarBorrado(id){
  		axios.get('http://localhost/restaurantes-vue/api-rest/restaurantes-api.php/delete-restaurante/'+id)
  			 .then((respuesta) => {
  			 	this.showBorrar = null;
  			 	this.getRestaurantes();
  			 });
